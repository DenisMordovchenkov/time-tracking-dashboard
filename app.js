...
class DashboardItem {
    ...
    constructor(data, container = '.dashboard__content', view = 'monthly') {
        this.data = data
    ...
    _createMarkup() {
        ...
        const id = title.toLowerCase().replace(/ /g, '-') //replace every space with '-'
        let infoCard = `<div class="dashboard__item dashboard__item--${id}">
      <article class="tracking-card">
        ...
        this.container.insertAdjacentHTML('beforeend', `${infoCard}`)

        this.time = this.container.querySelector(`.dashboard__item--${id} .tracking-card__time`)
        this.previousTime = this.container.querySelector(`.dashboard__item--${id} .tracking-card__prev-period`)
    }
    ...
    changeView(view) {
        ...
        this.previousTime.innerText = `Last ${DashboardItem.PERIODS[this.view]} - ${previous}hrs`
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    getDashboardData()
        .then(data => {
            const cards = data.map(el => new DashboardItem(el))
